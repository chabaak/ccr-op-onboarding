...
// pane chrome, the body and the corner grip, on u1's
// `.win / .win-ctl / .win-body / .win-grip` selectors. The reference
...
// nothing renders.
//
// A11y ([u3#c5]): the frame is a named region, the caption is a focusable move
// handle, and both controls are real buttons with accessible names.
...
export function buildWindowFrame(def: WindowDef): WindowFrame {
  ...
  const caption = el('div', 'win-caption')
  caption.tabIndex = 0
  // The caption is the window's ONE geometry handle from the keyboard: arrow keys
  ...
  const title = el('span', 'win-caption-label', def.caption)
  ...
  // window body scrolls past it. The keyboard path is Shift+arrow on the
  // caption, which announces that path in its own accessible name.
  ...
  const grip =
    def.resizable === false ? null : button('win-grip', `${def.en} 창 크기 조절 — 캡션에서 Shift+방향키`, '')
  if (grip) grip.tabIndex = -1

  root.append(caption, body, ...(grip ? [grip] : []))
  return { def, root, caption, body, grip, collapse, close }
}
