...
| The proxy accepts exactly one production origin; direct Node `fetch` and `localhost:5173` remain `403 origin_forbidden`. The loopback-only local relay now forwards only `/dday/*` with that configured origin, so the browser's live path can be smoke-tested without widening production CORS or committing model prose | 윤석, self | **resolved 08-16** — run `npm run dev:api` then `npm run smoke:local-api`; fixture mode remains the default when no local config exists | `tools/local-api-proxy.mjs` · `tools/local-api-smoke.mjs` · `proxy/src/handler.ts` |
